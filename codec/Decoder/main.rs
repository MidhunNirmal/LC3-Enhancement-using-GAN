...
fn main() -> Result<(), MainError> {
    ...
    */
     ...
     */
     ...
     let folder_path = "C:/Users/Midhun/Desktop/New folder (7)/project2/project2/clean_trainset_lc3"; // Update the path to your LC3 files directory
     ...
     if let Ok(entries) = fs::read_dir(folder_path) {
         for entry in entries.flatten() {
             ...
             if let Some(extension) = file_path.extension() {
                 if extension == "lc3" {
                     let lc3_file_name = file_path.as_path().to_str().unwrap();
                     let wav_file_name = format!(
                         "C:/Users/Midhun/Desktop/New folder (7)/project2/project2/src/DEC/{}.wav",
                         file_path.file_stem().unwrap().to_str().unwrap()
                     );
                     decode_lc3_to_wav(
                         lc3_file_name,
                         wav_file_name.as_str(),
                         SamplingFrequency::Hz16000,
                         16,
                         2, // Adjust based on your LC3 file format
                         FrameDuration::TenMs,
                         70, // Adjust according to your needs
                     )?;
                 }
             }
         }
