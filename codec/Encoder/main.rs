...
fn main() -> Result<(), MainError> {
    ...
    */
    ...
    let folder_path = "C:/Users/Midhun/Desktop/SEGAN/data/clean_trainset_wav";
    let folder_path = "C:/Users/Midhun/Desktop/SEGAN/data/clean_trainset_wav";
    ...
    if let Ok(entries) = fs::read_dir(folder_path) {
        for entry in entries.flatten() {
            ...
            if let Some(extension) = file_path.extension() {
                if extension == "wav" {
                    let wav_file_name = file_path.as_path().to_str().unwrap();
                    let lc3_file_name = format!(
                        "./clean_trainset_lc3/{}.lc3",
                        file_path.file_stem().unwrap().to_str().unwrap()
                    );
                    encode_wav_to_lc3(
                        wav_file_name,
                        lc3_file_name.as_str(),
